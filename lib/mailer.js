...
async function sendTicketNotification(ticket) {
  ...
  console.log('[mail] Submission notification sent to:', recipients.join(', '), '| messageId:', info.messageId);
  return info;
}
...
async function sendResolutionNotification(ticket, oldStatus) {
  ...
  console.log('[mail] Resolution notification sent to:', recipients.join(', '), '| messageId:', info.messageId);
  return info;
}

async function testSMTP() {
  const transporter = getTransporter();
  if (!transporter) return { ok: false, error: 'SMTP not configured (missing env vars)' };
  try {
    await transporter.verify();
    return { ok: true, host: process.env.SMTP_HOST, user: process.env.SMTP_USER };
  } catch (err) {
    return { ok: false, error: err.message, host: process.env.SMTP_HOST, user: process.env.SMTP_USER };
  }
}
